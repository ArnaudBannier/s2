...
export class S2LengthOld
    extends S2BaseType
    implements S2HasClone<S2LengthOld>, S2HasCopy<S2LengthOld>, S2HasLerpWithCamera<S2LengthOld>
{
    readonly kind = 'length' as const;
    public value: number;
    ...
    clone(): S2LengthOld {
        return new S2LengthOld(this.value, this.space, this.locked);
    }

    copyIfUnlocked(other: S2LengthOld): this {
        if (this.locked) return this;
        return this.copy(other);
    }

    copy(other: S2LengthOld): this {
        if (this.value === other.value && this.space === other.space) return this;
        this.value = other.value;
        this.space = other.space;
        this.markDirty();
    ...
    lerp(state0: S2LengthOld, state1: S2LengthOld, t: number, camera: S2Camera): this {
        const space = state1.space;
    ...
    static lerp(state0: S2LengthOld, state1: S2LengthOld, t: number, camera: S2Camera): S2LengthOld {
        return new S2LengthOld().lerp(state0, state1, t, camera);
    }
